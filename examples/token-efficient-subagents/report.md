# ✦ Goldproof report — "make my Claude Code sub-agents (and sessions) more token-efficient"
...
> **This run dogfoods Goldproof** — it researches how to make sub-agents token-efficient, which is
> exactly Goldproof's own architecture. All 8 sources are official Anthropic / Claude Code docs.
...
packaging. (Goldproof doesn't invent skill-candidates to hit a quota; it reports what the sources support.)

---

## 📐 Behavior changes → one `CLAUDE.md` block

Ten findings are durable norms. Goldproof would propose this single diff (dry-run — nothing writes without `--apply`):
...
*Generated by [Goldproof](../../README.md) — dry-run, X disabled. Every claim carries a source + confidence.*
